...
from voxel.PDB import PDB_backbone
from voxel.volume import adjust_size, coords_to_volume, coords_center_to_zero, weights_to_volume


def visualize_pdb(pdb_id, p=5, v_size=32, num=1, weights=None,
                  max_radius=40, noise_treatment=True,type_dataset = 'bp'):
    ...
    # Plot
    plot_volume(volume, pdb_id, v_size, num, weights=weights,dataset = type_dataset)
...
def plot_volume(volume, pdb_id, v_size, num, weights=None,dataset='bp'):
    ...
    # Save
	if weights == None:
		plt.savefig('../Examples/+'+ dataset +'None/' + str(pdb_id) + '_' + str(v_size) + '_' +
                str(weights) + '_' + str(num) + '.pdf')
	else:
    	plt.savefig('../Examples/' + dataset +'/'+ weights + '/' + str(pdb_id) + '_' + str(v_size) + '_' +
                str(weights) + '_' + str(num) + '.pdf')
...
'''
	...
	d = 'bp'
    visualize_pdb('2Q3Z', p=0, v_size=32, weights='hydropathy',type_dataset=d)
    visualize_pdb('2Q3Z', p=0, v_size=32, weights='charge', type_dataset=d)
    visualize_pdb('2Q3Z', p=0, v_size=32, weights='isoelectric', type_dataset=d)
    visualize_pdb('2Q3Z', p=0, v_size=32, weights=None, type_dataset=d)
